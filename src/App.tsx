...
function ThemeToggle() {
  ...
  return (
    <button onClick={() => setTheme(isDark ? 'light' : 'dark')} className="inline-flex items-center gap-2 rounded-2xl px-3 py-1.5 text-sm border border-slate-500 dark:border-slate-700 bg-white/60 dark:bg-[#111111] text-slate-700 dark:text-slate-200 hover:bg-white/80 dark:hover:bg-[#141414]" aria-label="Cambiar tema">
...
const DATA = {
  ...
  experience: [
    {
      company: 'Diseño Freelance',
      role: 'Diseñador Gráfico / Contenido',
      period: '04/2025 – Actualidad · Panamá',
      bullets: [
        'Diseño de identidad visual, piezas para redes y catálogos para microempresas.',
        'Optimización de formatos para impresión y digital con foco en conversión.'
      ]
    },
    {
      company: 'IDAAN',
      role: 'Diseñador Gráfico',
      period: '05/2022 – 04/2025 · Panamá',
      bullets: [
        'Desarrollo de campañas institucionales y material educativo para concienciación.',
        'Producción de artes para redes, prensa y formatos impresos a gran escala.'
      ]
    }
  ]
}
...
const Hero = () => (
  <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
    <div className="relative overflow-hidden rounded-2xl p-6 md:p-10 bg-gradient-to-br from-slate-100 to-slate-50 dark:from-[#0F0F0F] dark:to-[#111111] border border-slate-200 dark:border-slate-800">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div>
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            <a href={`https://wa.me/507${DATA.phone.replace(/[^0-9]/g, '').slice(-8)}`} target="_blank" rel="noreferrer">
              <Button className="rounded-2xl shadow-sm" style={{ backgroundColor: '#128C7E', color: 'white' }}>
                <img src="/icons/whatsapp.png" alt="WhatsApp" className="w-5 h-5" /> WhatsApp
              </Button>
            </a>
            <a href={`mailto:${DATA.email}`}>
              <Button className="rounded-2xl shadow-sm" style={{ backgroundColor: '#B22222', color: 'white' }}>
                <img src="/icons/gmail.png" alt="Gmail" className="w-5 h-5" /> Contáctame
              </Button>
            </a>
            <a href={DATA.cvUrl} target="_blank" rel="noreferrer">
              <Button className="rounded-2xl shadow-sm" style={{ backgroundColor: '#0A3D91', color: 'white' }}>
                <img src="/icons/download.png" alt="Descargar" className="w-5 h-5" /> Descargar CV
              </Button>
            </a>
          </div>
        </div>
        ...
        <Card key={i}>
          <CardContent className="p-4 md:p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className="text-lg font-semibold text-slate-900 dark:text-white">{e.role} · {e.company}</h3>
                <p className="text-sm text-slate-600 dark:text-slate-300">{e.period}</p>
              </div>
            </div>
    ...
    <section id="proyectos" className="mt-10">
      ...
      <Card className="rounded-2xl overflow-hidden">
        <div className="relative aspect-[16/9] select-none" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
          ...
          ) : (<div className="absolute inset-0 bg-slate-200 dark:bg-[#141414]" />)}
...
const Footer = () => (<footer className="mt-16 pb-10 text-center text-sm text-slate-500 dark:text-slate-400">© {new Date().getFullYear()} José Enrique Scott. Hecho con ♥ en Panamá.</footer>)

export default function App(){return (<main className="mx-auto max-w-6xl px-4 py-10 bg-white dark:bg-[#0B0B0B] text-slate-900 dark:text-slate-100"><Hero/><Projects/><Skills/><Experience/><Footer/></main>)}